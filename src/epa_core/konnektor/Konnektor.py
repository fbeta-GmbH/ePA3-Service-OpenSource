...
from epa_core.utils.utils import convert_der_ecdsa_to_concated_x962
from epa_core.runtime_config.constants import Config

from epa_core.exceptions import KonnektorException, CardException, ErrorCodes
from epa_core.xml_service.soap_client import SoapClient

from epa_core.konnektor.pkcs12adapter import PinnedPkcs12Adapter
...
class Konnektor:
    def __init__(self, path_to_p12:str, test_conn: bool = True)->None:
        ...
        """
        Config.logger.info("Initializing Konnektor with p12 certificate: %s", path_to_p12)
        self.path_to_p12 = path_to_p12
        self.cert_password = Config.KONNEKTOR_CERT_PW

        self.session = Session()

        ca_bundle = Config.KONNEKTOR_CA_BUNDLE if isinstance(Config.KONNEKTOR_CA_BUNDLE, str) else None
                
        pkcs12_adapter = PinnedPkcs12Adapter(
            pkcs12_filename=self.path_to_p12,
            pkcs12_password=self.cert_password,
            ca_bundle=ca_bundle,
            tls_hostname=Config.KONNEKTOR_TLS_HOSTNAME,
            target_ip=Config.KONNEKTOR_IP,
        )
        ...
        if Config.KONNEKTOR_CA_BUNDLE:
            self.session.verify = Config.KONNEKTOR_CA_BUNDLE
        else:
        ...
        try:
            ...
            Config.logger.error("Failed to initialize Konnektor connection: %s", str(e))
            raise
    ...
    def test_connection(self)->bool:
        ...
        try:
            response = self.session.get(
                f'{Config.KONNEKTOR_URL}/connector.sds',
                timeout=Config.HTTPS_TIMEOUT
            )
            if response.status_code == status.HTTP_200_OK:
                Config.logger.info("Konnektor connection test successful")
                return True
            else:
                Config.logger.error("Konnektor connection test failed with status code: %d", response.status_code)
                return False     
    ...
    def clear_card_data(self)->None:
        ...
        if card_env:
            Config.logger.debug("Cleared card handle from environment variables.")
        if cert_env:
            Config.logger.debug("Cleared card certificate from environment variables.")

        c = self.db.cursor()
        c.execute("SELECT card, card_certificate FROM card_data")
        rows = c.fetchall()
        if rows:
            Config.logger.debug(f"Clearing {len(rows)} entries of card data from database.")
    ...
    def get_cards(self)->str:
        ...
        """
        try:
            Config.logger.info("Getting cards")

            soap_request = SoapClient.generate_xml(
                SoapClient.Services.EventService.EventServicePort.GetCards, 
                params={
                    "Context": {
                        "MandantId": Config.MANDANT_ID,
                        "ClientSystemId": Config.CLIENT_SYSTEM_ID,
                        "WorkplaceId": Config.WORKPLACE_ID,
                    }
                }
            )

            response = self.session.post(
                f'{Config.KONNEKTOR_URL}/webservices/eventservice',
                data=soap_request,
                timeout=Config.HTTPS_TIMEOUT
            )
            ...
            if parsed_response.get('Cards') is not None:
                for card in parsed_response.get('Cards', {}).get('Card', []):
                    if card['CardType'] == "SMC-B":
                        ...
                        Config.logger.debug("CardHandle: %s", card_handle)
    ...
    def read_card_certificate(self, card_handle: str) -> str:
        ...
        """
        try:
            Config.logger.info("Reading card certificate for card handle: %s", card_handle)
            soap_request = SoapClient.generate_xml(
            ...
            response = self.session.post(
                f'{Config.KONNEKTOR_URL}/webservices/certificateservice',
                data=soap_request,
                timeout=Config.HTTPS_TIMEOUT
            )
    ...
    def is_card_pin_verified(self, card_handle: str) -> bool:
        ...
        """
        try:
            Config.logger.info("Getting PIN status for card handle: %s", card_handle)
            soap_request = SoapClient.generate_xml(
            ...
            response = self.session.post(
                f'{Config.KONNEKTOR_URL}/webservices/cardservice',
                data=soap_request,
                timeout=Config.HTTPS_TIMEOUT
            )
            ...
            if pin_status == 'VERIFIABLE':
                Config.logger.info(f"PIN not verified for card handle: {card_handle}")
                return False
            elif pin_status == 'VERIFIED':
                Config.logger.info(f"PIN verified for card handle: {card_handle}")
                return True
            else:
                raise CardException(
    ...
    def create_signed_attest_jwt(self, nonce:str, card_handle:str, card_certificate:str)->str:
        ...
        Config.logger.info("Creating signed attest JWT")
        ...
        Config.logger.debug("Client attest: %s", client_attest)
        ...
        Config.logger.debug("Binary string (base64): %s", binary_string_base64)
        ...
        Config.logger.debug("Final JWT: %s", final_jwt)
    ...
    def get_challenge_token_signature(self, header_payload_challenge_string:str, card_handle:str)->str:
        ...
        """
        Config.logger.info("Signing challenge token")

        Config.logger.debug("Header payload challenge string: %s", header_payload_challenge_string)
        binary_string = hashlib.sha256(header_payload_challenge_string.encode('utf-8')).digest()
        Config.logger.debug("Binary string: %s", binary_string)
        binary_hex = binary_string.hex()
        Config.logger.debug("Binary string (hex): %s", binary_hex)
        binary_string_base64 = base64.b64encode(bytes.fromhex(binary_hex)).decode('utf-8')
        Config.logger.debug("Binary string (base64): %s", binary_string_base64)
        challenge_signature = self.external_authenticate(card_handle, binary_string_base64, signature_type="urn:bsi:tr:03111:ecdsa")

        Config.logger.debug("Signature: %s", challenge_signature)
    ...
    def external_authenticate(self, card_handle:str, challenge:str, signature_type:str="urn:ietf:rfc:3447")->str:
        ...
        """
        try:
            Config.logger.info("Performing ExternalAuthenticate for card handle: %s", card_handle)
            soap_request = SoapClient.generate_xml(
            ...
            response = self.session.post(
                f'{Config.KONNEKTOR_URL}/webservices/authsignatureservice',
                data=soap_request,
                headers={
            ...
            Config.logger.debug("ExternalAuthenticate Response: %s", response.text)
            ...
            if parsed_response['Status']['Result'] != 'OK' or parsed_response['Status']['Error'] != None:
                if parsed_response['Status']['Error'] == "Karte nicht als gesteckt identifiziert":
                    ...
                    Config.logger.error("Error during ExternalAuthenticate: %s", parsed_response['Status'])
                    raise KonnektorException(
            ...
            if type_is_ecdas and base64_signature:
                try:        
                    Config.logger.debug("Before convert signature to X962: %s", base64.urlsafe_b64encode(base64_signature))
                    base64_signature = convert_der_ecdsa_to_concated_x962(base64_signature)
                    Config.logger.debug("Converted signature to X962: %s", base64_signature)
                except Exception as e:
                    raise KonnektorException(
            ...
            Config.logger.debug("Base64Signature: %s", base64_signature)
