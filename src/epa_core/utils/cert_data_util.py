...
def extract_from_p12(p12_path, password, cert_out_path, key_out_path):
    ...
    if certificate:
        with open(cert_out_path, "w") as cert_file:
            # Nur die wichtigen Informationen
            subject = certificate.subject.rfc4514_string()
            issuer = certificate.issuer.rfc4514_string()
            cert_file.write(f"subject={subject}\n")
            cert_file.write(f"issuer={issuer}\n")
            
            # Das eigentliche Zertifikat im PEM-Format
...
#     # Beispielaufruf
#     p12_path = os.path.join(os.path.dirname(__file__), "KVS_Client_Test.p12")
#     cert_out_path = os.path.join(os.path.dirname(__file__), "cert.pem")
#     key_out_path = os.path.join(os.path.dirname(__file__), "key.pem")
#     password = "0000"

#     extract_from_p12(p12_path, password, cert_out_path, key_out_path)
